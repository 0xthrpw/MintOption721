...
*/
describe('Option721', function() {
  let alice, bob, carol, dev;
  let Tiny721, Option721, MockERC721Receiver;
  before(async () => {
  ...
  const METADATA_URI = '';
  const CAP = 10420;
  let tiny721, option721, goodReceiver, badReceiver;
  beforeEach(async () => {
    ...
    option721 = await Option721.connect(alice.signer).deploy(
      NAME,
      SYMBOL,
      "MINT OPTIONAL",
      CAP,
  ...
  context('with no minted tokens', async function() {
    it('has 0 totalSupply', async function() {
      const supply = await option721.totalSupply();
  ...
  context('with minted tokens', async function() {
    beforeEach(async function() {
      await option721.connect(alice.signer).mintOpt(1, 0, alice.address);
      await option721.connect(alice.signer).mintOpt(2, 0, bob.address);
      await option721 .connect(alice.signer).mintOpt(3, 0, carol.address);
    });

    // Confirm that we can retrieve balance of holder tokens.
    describe('balanceOf', async function() {
      it('returns the amount for a given address', async function() {
        let aliceBalance = await option721.connect(alice.signer).balanceOf(
          alice.address);
        let bobBalance = await option721.connect(alice.signer).balanceOf(
          bob.address);
        let carolBalance = await option721.connect(alice.signer).balanceOf(
          carol.address);
        let devBalance = await option721.connect(alice.signer).balanceOf(
          dev.address);
        let zeroBalance = await option721.connect(alice.signer).balanceOf(
    ...
    describe('ownerOf', async function() {
      it('returns the right owner', async function() {
        let ownerOne = await option721.connect(alice.signer).ownerOf(1);
        ownerOne.should.be.equal(alice.address);
        let ownerTwo = await option721.connect(alice.signer).ownerOf(2);
        ownerTwo.should.be.equal(bob.address);
        let ownerFour = await option721.connect(alice.signer).ownerOf(4);
      ...
      it('reverts for an invalid token', async function() {
        await expect(
          option721.connect(alice.signer).ownerOf(0)
        ).to.be.revertedWith('OwnerQueryForNonexistentToken');
        await expect(
          option721.connect(alice.signer).ownerOf(7)
        ).to.be.revertedWith('OwnerQueryForNonexistentToken');
      });
    });
    ...
    describe('approve', async function() {
      it('sets approval for the target address', async function() {
        await option721.connect(alice.signer).approve(bob.address, 1);
        const approval = await option721.connect(alice.signer).getApproved(1);
        approval.should.be.equal(bob.address);
        let ownerOne = await option721.connect(alice.signer).ownerOf(1);
        ownerOne.should.be.equal(alice.address);
        await option721.connect(bob.signer).transferFrom(alice.address,
          bob.address, 1);
        ownerOne = await option721.connect(alice.signer).ownerOf(1);
      ...
      it('rejects an unapproved caller', async function() {
        await expect(
          option721.connect(bob.signer).approve(bob.address, 1)
      ...
      it('does not get approved for invalid tokens', async function() {
        await expect(
          option721.connect(alice.signer).getApproved(7)
    ...
    describe('setApprovalForAll', async function() {
      it('sets approval for all properly', async function() {
        const approvalTx = await option721.connect(alice.signer)
          .setApprovalForAll(bob.address, true);
        await expect(approvalTx)
          .to.emit(option721, 'ApprovalForAll')
          .withArgs(alice.address, bob.address, true);
        let isBobApproved = await option721.isApprovedForAll(alice.address,
          bob.address);
    ...
    context('test transfer functionality', async function() {
      ...
      */
      const testTransfer = async function(functionSignature) {
        let args, transfer;
        beforeEach(async function() {
          await option721.connect(alice.signer).setApprovalForAll(carol.address,
            true);
          ...
          transfer = await option721.connect(alice.signer)[functionSignature]
            .apply(this, args);
        });

        // Confirm that balances appropriately reflect the transfer happening.
        it('adjusts owners balances', async function() {
          let aliceBalance = await option721.connect(alice.signer)
            .balanceOf(alice.address);
          let bobBalance = await option721.connect(alice.signer)
        ...
        it('transfers the ownership of the given token ID to the given address', async function() {
          let tokenOwner = await option721.connect(alice.signer).ownerOf(1);
          tokenOwner.should.be.equal(bob.address);
        });

        // Any token approval that ALice may have set should be cleared.
        it('clears the approval for the token ID', async function() {
          let tokenApproval = await option721.connect(alice.signer)
            .getApproved(1);
        ...
        */
        it('rejects unapproved transfer', async function() {
          await expect(
            option721.connect(alice.signer)[functionSignature]
              .apply(this, args)
        ...
        */
        it('reject transfer from operator for unowned id', async function() {
          args[2] += 1;
          await expect(
            option721.connect(carol.signer)[functionSignature]
              .apply(this, args)
          ).to.be.revertedWith('TransferCallerNotOwnerNorApproved');
        });
        ...
        it('rejects transfer from incorrect owner', async function() {
          await expect(
            option721.connect(bob.signer)[functionSignature]
              .apply(this, args)
        ...
        */
        it('rejects transfer to zero address', async function() {
          ...
          args[2] = 2;
          await expect(
            option721.connect(bob.signer)[functionSignature].apply(this, args)
        ...
        */
        if (functionSignature.startsWith('safe')) {
          it('validates ERC721Received on safe transfers', async function() {
            ...
            args[2] = 2;
            let safeTransfer = await option721.connect(bob.signer)[functionSignature].apply(this, args);
  ...
  context('mint', async function() {
    ...
    it('successfully mints multiple tokens', async function() {
      ...
      for (let tokenId = 0; tokenId < 5; tokenId++) {
        ...
        await expect(mint).to.not
          .emit(goodReceiver, 'Received');
        let newOwner = await option721.ownerOf(1 + tokenId);
        newOwner.should.be.equal(alice.address);
      }
    });

    it('does not revert for non-receivers', async function() {
      await option721.mintOpt(1, 0, option721.address);
      let ownerOne = await option721.ownerOf(1);
      ownerOne.should.be.equal(option721.address);
    });

    it('rejects mints to the zero address', async function() {
      await expect(
        option721.connect(alice.signer).mintOpt(1, 0, ethers.constants.AddressZero)
      ).to.be.revertedWith('MintToZeroAddress');
    });

    it('requires quantity to be greater than 0', async function() {
      await expect(
        option721.connect(alice.signer).mintOpt(0, 0, alice.address)
      ).to.be.revertedWith('MintZeroQuantity');
    });

    it('locks transfers', async function() {
      const mint = await option721.mintOpt(1, 0, alice.address);
      await option721.connect(alice.signer).lockTransfer(1, true);

      await expect(
        option721.connect(alice.signer).transferFrom(alice.address, bob.address, 1)
      ).to.be.revertedWith('TransferIsLocked');
    });

    it('returns tokenUri', async function() {
      const mint = await option721.mintOpt(1, 0, alice.address);
      await option721.connect(alice.signer).tokenURI(1);
