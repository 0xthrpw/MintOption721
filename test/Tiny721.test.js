...
describe('Tiny721', function() {
  ...
  context('mint', async function() {
    ...
    it('locks transfers', async function() {
      const mint = await tiny721.mint_Qgo(alice.address, 1);
      await tiny721.connect(alice.signer).lockTransfer(1, true);

      await expect(
        tiny721.connect(alice.signer).transferFrom(alice.address, bob.address, 1)
      ).to.be.revertedWith('TransferIsLocked');
    });

    it('returns tokenUri', async function() {
      const mint = await tiny721.mint_Qgo(alice.address, 1);
      await tiny721.connect(alice.signer).tokenURI(1);
    });
  });
});
